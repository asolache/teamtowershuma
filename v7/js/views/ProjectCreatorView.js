...
export default class ProjectCreatorView {
    ...
    async getHtml() {
        ...
        // 1. CARGAR CATÁLOGO DE PLANTILLAS
        let sectorOptions = '';
        
        // Plantillas Custom (Si el usuario ha creado alguna)
        ...
        // Plantillas Nativas (Los 9 Sectores de GLOBAL_ONTOLOGY)
        sectorOptions += `<optgroup label="📦 Catálogo Nativo (Los 9 Ecosistemas)">`;
    ...
    executeViewScript() {
        ...
        // 3. PARSER NATIVO DE LA PLANTILLA A LA RED V10
        this.dom.btnLoadTemplate.addEventListener('click', () => {
            ...
            if (sectorData) {
                // Filtramos _meta en caso de que exista
                const roleKeys = Object.keys(sectorData).filter(k => k !== '_meta' && k !== 'roles');
                ...
                roleKeys.forEach(levelKey => {
                    ...
                    this.draftRoles.push({
                        ...
                        ai_prompt: data.ai_prompt || data.ai_prompt || '' 
                    });
                    ...
                    if (data.standard_deliverables) {
                        data.standard_deliverables.forEach(deliv => {
                            ...
                            this.draftTxs.push({
                                ...
                                toLevel: toLevel,
                                tipo: tipo,
                                template: deliv.name, // V10 nomenclature
    ...
    async finalizeProject() {
        ...
        this.dom.btnLaunch.innerText = 'Creando Tuberías V10...';

        // 1. CREATE_PROJECT: Instancia base V10
        ...
        if (p && this.draftTxs && this.draftTxs.length > 0) {
            for (const aiTx of this.draftTxs) {
                ...
                if (roleFrom && roleTo) {
                    // V10: Creador de Tuberías permanentes desde la IA/Plantilla
                    await store.dispatch({
                        type: 'ADD_FLOW',
                        payload: {
                            projectId: projectId,
        ...
        // Fija el proyecto como activo y manda al Kanban Oficial
