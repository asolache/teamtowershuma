...
// Rutas relativas · Zero /v9/ · KB-first para estado colapsado
// =============================================================================
    ...
    static getHtml(currentPath = '') {
        ...
        <style>
            .sidebar {
                ...
                background: var(--bg-panel, rgba(15,15,20,0.95));
                border-right: 1px solid var(--glass-border);
                display: flex; flex-direction: column;
                transition: width 0.3s cubic-bezier(0.2,0.8,0.2,1);
                z-index: 100; position: relative; flex-shrink: 0; overflow-y: auto;
            }
            ...
            .sb-brand-icon { font-size: 1.8rem; display: none; text-shadow: 0 0 15px rgba(99,102,241,0.6); }
            ...
            .sb-eco-label { font-size: 0.7rem; color: var(--accent-indigo, #6366f1); text-transform: uppercase; font-weight: bold; letter-spacing: 1px; margin-bottom: 5px; display: block; }
            .sidebar.minimized .sb-eco-label { display: none; }

            .sb-eco-select { width: 100%; background: rgba(0,0,0,0.5); border: 1px solid var(--accent-indigo, #6366f1); color: white; font-size: 0.85rem; padding: 8px 10px; border-radius: 8px; font-weight: bold; cursor: pointer; outline: none; transition: 0.3s; text-overflow: ellipsis; white-space: nowrap; overflow: hidden; }
            .sb-eco-select:focus { box-shadow: inset 0 0 10px rgba(99,102,241,0.2); }
            .sidebar.minimized .sb-eco-select { display: none; }

            .sb-eco-icon-min { display: none; color: var(--accent-indigo, #6366f1); font-size: 1.2rem; cursor: pointer; text-decoration: none; }
            .sidebar.minimized .sb-eco-icon-min { display: block; }
            .sb-eco-icon-min:hover { transform: scale(1.1); }

            .sb-nav { flex: 1; padding: 1rem 0; display: flex; flex-direction: column; gap: 4px; overflow-y: auto; overflow-x: hidden; }
            ...
            .nav-item.active { background: rgba(99,102,241,0.1); color: var(--accent-indigo, #6366f1); border-left-color: var(--accent-indigo, #6366f1); font-weight: 900; }
            ...
            .sidebar.minimized .nav-item.active { border-left: 3px solid var(--accent-indigo, #6366f1); }
            ...
            .sb-avatar { width: 35px; height: 35px; border-radius: 50%; background: linear-gradient(135deg, var(--accent-indigo, #6366f1), var(--accent-purple, #e040fb)); color: white; display: flex; justify-content: center; align-items: center; font-weight: 900; font-size: 1.1rem; flex-shrink: 0; box-shadow: 0 0 10px rgba(99,102,241,0.3); }
            ...
            .sb-user-role { color: var(--accent-green, #00e676); font-size: 0.7rem; font-family: var(--font-mono); text-transform: uppercase; font-weight: bold; letter-spacing: 1px; }
        ...
        <nav class="sidebar ${isMin ? 'minimized' : ''}" id="mainSidebar">
            <div class="sb-header">
                <a href="/" data-link class="sb-brand" id="btnSidebarLogo" title="Volver a la Matriz Global">
                    <span class="sb-brand-icon">🗼</span>
                    <img src="https://raw.githubusercontent.com/asolache/teamtowershuma/main/v9/logoteamtowers.png"
                         alt="TeamTowers" class="sb-brand-img">
                </a>
            ...
            <div class="sb-ecosystem-wrapper">
                ...
                <a href="/dashboard" data-link class="sb-eco-icon-min" title="${projectLabel}">🌐</a>
            </div>
            ...
            <div class="sb-footer">
                ...
                <div class="sb-user-info">
                    ...
                    <span class="sb-user-role">${(state.session.role || 'guest').replace('-', ' ')}</span>
                </div>
            </div>
        </nav>`;
    }
    ...
    static initListeners() {
        ...
        if (btnToggle && sidebar) {
            btnToggle.addEventListener('click', (e) => {
                ...
                localStorage.setItem('tt_sidebar_min', sidebar.classList.contains('minimized'));
            });
        }
        ...
        if (selectProject) {
            selectProject.addEventListener('change', (e) => {
                ...
                if (val) {
                    localStorage.setItem('tt_active_project', val);
