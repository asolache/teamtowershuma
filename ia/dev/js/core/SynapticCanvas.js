    ...
    async _initWebGLGraph() {
        ...
        setTimeout(() => {
            try {
                ...
                // Distancia de links según tipo — swarm más corto que KB
                this.graph3D.d3Force('link')
                    ?.distance(link => {
                        if (link.tipo === 'swarm-member')   return 120;  // agente cerca del orquestador
                        if (link.tipo === 'orchestration')  return 200;  // orquestador cerca del kernel
                        if (link._isVnaLink)                return 180;
                        if (link.isDependencies)            return 80;
                        return 150;
                    })
                    ?.strength(link => {
                        if (link.tipo === 'swarm-member')  return 0.8;  // atracción fuerte al orch
