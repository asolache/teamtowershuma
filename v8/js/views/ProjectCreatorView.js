...
export default class ProjectCreatorView {
    constructor() {
        ...
        this.guardians = [
            ...
            { id: 'caregiver', label: '❤️ Cuidador' },
            { id: 'ruler', label: '👑 Gobernante' },
            { id: 'jester', label: '🃏 Bufón' },
            { id: 'everyman', label: '🤝 Ciudadano' },
            { id: 'lover', label: '🔥 Amante' },
            { id: 'hero', label: '⚔️ Héroe' },
            { id: 'outlaw', label: '🏴‍☠️ Rebelde' },
            { id: 'magician', label: '✨ Mago' },
            { id: 'innocent', label: '🕊️ Inocente' },
    ...
    async getHtml() {
        ...
        let sectorOptions = `<optgroup label="📦 Catálogo Nativo (V8 Core)">`;
        Object.keys(NATIVE_ONTOLOGY).forEach(k => {
            const sectorName = k.replace(/_/g, ' ').toUpperCase();
            sectorOptions += `<option value="native_${k}" ${preselectedSector === k ? 'selected' : ''}>${sectorName}</option>`;
        });
                    ...
                    <div class="wizard-card">
                        ...
                        <div id="step1">
                            <div class="wizard-header">
                                ...
                                <p>Genera una red neuronal de valor a partir de plantillas o IA.</p>
                            </div>
                            ...
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-bottom: 2rem;">
                                ...
                                <div class="form-group" style="margin: 0;">
                                    <label>ADN Estructural (Plantilla)</label>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="inpVision" class="lux-input vision-box" placeholder="Describe brevemente la idea. El Orquestador IA deducirá los roles necesarios y trazará las tuberías de valor tangibles e intangibles..."></textarea>
                            </div>
                            ...
                            <div class="actions-row">
                                ...
                                <button class="btn-lux btn-lux-outline" id="btnLoadTemplate">🏗️ Clonar Plantilla</button>
                                <button class="btn-lux btn-lux-primary" id="btnGenerateAI">🧠 Diseñar Topología IA</button>
                            </div>
                        </div>
                        ...
                        <div id="step2" style="display: none;">
                            ...
                            <div id="aiTxFeedback" class="tx-feedback-box" style="display: none;" title="Haz clic para ver un adelanto de las tuberías y el Pitch">
                                <div>
                                    ...
                                    <div style="color: var(--text-muted); font-size: 0.9rem;">El Agente IA ha trazado <strong id="txCount" style="color: white; font-size: 1.2rem; font-family: monospace;">0</strong> flujos base (Clic para previsualizar).</div>
                                </div>
    ...
    executeViewScript() {
        ...
        this.dom.btnLoadTemplate.addEventListener('click', () => {
            ...
            if (sectorVal.startsWith('native_')) {
                const key = sectorVal.replace('native_', '');
                sectorData = NATIVE_ONTOLOGY[key];
            }
            ...
            this.draftPresentation = this.dom.inpVision.value.trim() || `Red instanciada con plantilla: ${this.dom.inpSector.options[this.dom.inpSector.selectedIndex].text}.`;
            
            if (sectorData) {
                const roleKeys = Object.keys(sectorData);

                roleKeys.forEach(levelKey => {
                    const data = sectorData[levelKey];
