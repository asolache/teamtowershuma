    ...
    async callLLM({ provider, apiKey, systemPrompt, userPrompt, responseFormat = "json_object", temperature = 0.2, maxRetries = 2 }) {
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                // 🔥 FIX SHIELDS: Si el error es de red pura (CORS bloqueado), avisamos al usuario en vez de dar 404
                if (error.message.includes("Failed to fetch") || error.name === "TypeError") {
                    console.warn("⚠️ [Orquestador] Petición abortada por el navegador. Posible bloqueo CORS/AdBlocker.");
                    if (attempt > maxRetries) {
                        throw new Error("El navegador ha bloqueado la conexión con la IA (CORS). Desactiva el Bloqueador de Anuncios, los 'Brave Shields' o la 'Protección de Rastreo' de Firefox para esta web.");
                    }
                } else {
