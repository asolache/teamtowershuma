...
export const Sidebar = {
        ...
        return `
            ...
            <aside class="sidebar ${collapsedClass}" id="mainSidebar">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 1.5rem; width: 100%; box-sizing: border-box;">
                    <a href="/v8/" class="logo-area" data-link style="display: flex; align-items: center; text-decoration: none; overflow:hidden;">
                        ...
                        <div>
                            ...
                            <div style="color: var(--accent-blue); font-family: var(--font-mono); font-size:0.7rem; font-weight:bold; letter-spacing:1px; margin-top:2px;">KERNEL V15.5</div>
                        </div>
                    </a>
                ...
                <div style="display: flex; flex-direction: column; flex: 1; overflow-y: auto; overflow-x: hidden; width: 100%;">
                    ...
                    <a href="/v8/" class="side-link ${currentPath === '/' ? 'active' : ''}" data-link title="Mis Ecosistemas"><span class="link-icon">🌐</span> <span class="link-text">Mis Ecosistemas</span></a>
                    <a href="/v8/settings" class="side-link ${currentPath === '/settings' ? 'active' : ''}" data-link title="Panteón Global (Gobernanza)"><span class="link-icon">🏛️</span> <span class="link-text">Panteón Global</span></a>

                    <div class="side-category">Operativa Local (Castell)</div>
                    <a href="/v8/dashboard" class="side-link ${currentPath === '/dashboard' ? 'active' : ''}" data-link title="Radar (Macro-Vista)"><span class="link-icon">🛰️</span> <span class="link-text">Radar VNA</span></a>
                    <a href="/v8/map" class="side-link ${currentPath === '/map' ? 'active' : ''}" data-link title="Topología (Diseño)"><span class="link-icon">🕸️</span> <span class="link-text">Topología</span></a>
                    <a href="/v8/lms" class="side-link ${currentPath === '/lms' ? 'active' : ''}" data-link title="La Forja (Cerebro LMS)"><span class="link-icon">🧠</span> <span class="link-text">La Forja (LMS)</span></a>
                    <a href="/v8/agents" class="side-link ${currentPath === '/agents' ? 'active' : ''}" data-link title="Padrón Neuronal"><span class="link-icon">🧪</span> <span class="link-text">Padrón Neuronal</span></a>
                    <a href="/v8/project" class="side-link ${currentPath === '/project' ? 'active' : ''}" data-link title="Mercado PULL (Kanban)"><span class="link-icon">📋</span> <span class="link-text">Mercado PULL</span></a>
                    <a href="/v8/paper" class="side-link ${currentPath === '/paper' || currentPath === '/focus' ? 'active' : ''}" data-link title="Omni-Paper (Usenet)"><span class="link-icon">📝</span> <span class="link-text">Omni-Paper</span></a>
                    <a href="/v8/ledger" class="side-link ${currentPath === '/ledger' ? 'active' : ''}" data-link title="Notaría y Equity"><span class="link-icon">⚖️</span> <span class="link-text">Notari Ledger</span></a>

                    <div class="side-category">Sistema</div>
                    <a href="/v8/profile" class="side-link ${currentPath === '/profile' ? 'active' : ''}" data-link title="Mi Ikigai (Perfil)"><span class="link-icon">👤</span> <span class="link-text">Mi Perfil (SBTs)</span></a>
                </div>

                <div style="margin-top: auto; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.05); width: 100%;">
                    <button id="btnSideLogout" class="btn-logout" title="Suspender sesión del Nodo">
    ...
    initListeners: () => {
        ...
        // LOGOUT BLINDADO PARA SIDEBAR
        const btnSideLogout = document.getElementById('btnSideLogout');
        if (btnSideLogout) {
            btnSideLogout.addEventListener('click', async (e) => {
                e.preventDefault();
                e.stopPropagation();
                try {
                ...
                try {
                    const rawState = localStorage.getItem('tt_v9_kernel_state');
                    if (rawState) {
                        const parsed = JSON.parse(rawState);
                        parsed.session.activeUserId = null;
                        parsed.session.role = 'guest';
                        localStorage.setItem('tt_v9_kernel_state', JSON.stringify(parsed));
                    }
                    localStorage.removeItem('tt_active_project');
                } catch(err) {}
                
                window.location.assign('/v8/'); 
            });
        }
    }
};
