...
export const PageHeader = {
    getHtml: (config = {}) => {
        ...
        const tabsHtml = config.tabs ? `
            <div class="ph-tabs">
                ${config.tabs.map(tab => `
                    <button class="ph-tab ${tab.active ? 'active' : ''}" data-target="${tab.id}">${tab.label} ${tab.badge ? `<span class="tab-badge">${tab.badge}</span>` : ''}</button>
                `).join('')}
            </div>
        ` : '';
        ...
        const magicActionsHtml = config.magicActions ? `
            <div class="ph-magic-menu">
                <button class="ph-btn-magic" id="btnMagicMenu">✨ Acciones IA ▾</button>
        ...
        return `
            <style>
                .page-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 2rem; position: relative; z-index: 100;}
                .ph-left { display: flex; flex-direction: column; gap: 5px; }
                .ph-title { color: white; font-size: 2rem; font-weight: 900; margin: 0; letter-spacing: -1px; display:flex; align-items:center; gap:10px;}
                ...
                .ph-tagline { color: var(--text-muted); font-size: 0.95rem; margin: 0; }
                ...
                .ph-tab { background: transparent; border: none; color: #888; padding: 8px 16px; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.3s; font-size: 0.9rem; display:flex; align-items:center; gap:8px;}
                .ph-tab:hover { color: white; background: rgba(255,255,255,0.05); }
                .ph-tab.active { background: rgba(255,255,255,0.1); color: white; }
                .tab-badge { background: var(--accent-red); color: white; font-size: 0.7rem; padding: 2px 6px; border-radius: 10px; font-family: monospace;}

                .ph-avatar-container { position: relative; cursor: pointer; }
                ...
                .ph-menu-item { display: flex; align-items: center; gap: 12px; padding: 12px 15px; color: #ccc; text-decoration: none; border-radius: 10px; transition: 0.2s; font-size: 0.9rem; font-weight: bold; border: 1px solid transparent; cursor:pointer;}
                ...
                .ph-btn-magic { background: rgba(224, 64, 251, 0.1); border: 1px solid rgba(224, 64, 251, 0.3); color: var(--accent-purple); padding: 10px 20px; border-radius: 12px; font-weight: 900; cursor: pointer; transition: 0.3s; font-size: 0.9rem;}
                .ph-btn-magic:hover { background: rgba(224, 64, 251, 0.2); box-shadow: 0 0 15px rgba(224, 64, 251, 0.3);}
                .ph-magic-dropdown { position: absolute; top: 50px; right: 0; background: rgba(10,10,15,0.95); border: 1px solid var(--accent-purple); border-radius: 16px; width: 280px; padding: 10px; box-shadow: 0 15px 40px rgba(0,0,0,0.8); backdrop-filter: blur(15px); display: none; flex-direction: column; gap: 5px;}
                ...
                .ph-magic-icon { font-size: 1.5rem; background: rgba(0,0,0,0.5); width: 40px; height: 40px; display: flex; justify-content: center; align-items: center; border-radius: 10px;}
                .ph-magic-text { display: flex; flex-direction: column; }
                .ph-magic-text strong { color: white; font-size: 0.9rem; }
                .ph-magic-text span { color: var(--accent-purple); font-size: 0.75rem; font-family: var(--font-mono); }
                ...
                @media (max-width: 1024px) {
                    ...
                    .ph-tabs { width: 100%; overflow-x: auto; }
                    .ph-tab { flex: 1; justify-content: center; white-space: nowrap;}
                }
            </style>
                ...
                <div class="ph-right">
                    ...
                    <div class="ph-avatar-container" id="phAvatarToggle">
                        ...
                        <div class="ph-user-menu" id="phUserMenu">
                            ...
                            <div class="ph-menu-item danger" id="btnLogoutUser">🚪 Desconectar Nodo</div>
                        </div>
                    </div>
                </div>
            </header>
    ...
    execute: () => {
        ...
        if (avatarToggle && userMenu) {
            ...
            document.addEventListener('click', (e) => {
                if (!avatarToggle.contains(e.target)) userMenu.classList.remove('open');
            });
        }
        ...
        if (btnLogout) {
            btnLogout.addEventListener('click', async () => {
                ...
                window.location.href = '/v8/'; // Redirige al Home/Zero-Trust
            });
        }
        ...
        if (btnMagic && magicDrop) {
            btnMagic.addEventListener('click', () => magicDrop.classList.toggle('open'));
            document.addEventListener('click', (e) => {
                if (!btnMagic.contains(e.target) && !magicDrop.contains(e.target)) magicDrop.classList.remove('open');
            });
