...
export const PageHeader = {
    getHtml: (config) => {
        ...
        // --- BLINDAJE DEL TAG VERDE (ARCHETYPE) ---
        const archetypeBadgeHtml = config.subtitle ? `
            <span class="ph-header-subtitle" style="white-space: nowrap; display: inline-block; vertical-align: middle;">
                ${config.subtitle}
            </span>
        ` : '';
        ...
        return `
            <style>
                /* --- BLINDAJE GLOBAL GLOBAL TOP BAR --- */
                .ph-global-top-bar { 
                    display: flex; 
                ...
                /* --- BLINDAJE VIEW HEADER (PC Responsivo) --- */
                .ph-view-header { 
                    margin-bottom: 2rem; 
                    display: flex; 
                    justify-content: space-between; 
                    align-items: flex-start; /* Cambio de end a start para mejor wrap */
                    gap: 20px;
                    flex-wrap: wrap; /* Permite que acciones bajen si no hay espacio */
                }
                .ph-view-header-title-group {
                    flex: 1;
                    min-width: 300px; /* Espacio mínimo para el título antes de wrap */
                    max-width: 100%;
                }
                .ph-view-header h1 { 
                    font-size: 2.2rem; 
                    ...
                    font-weight: 900;
                    line-height: 1.2;
                    word-wrap: break-word; /* Blindaje antipánico para títulos largos */
                }
                .ph-view-header p { color: var(--text-muted); font-size: 1rem; margin-top: 8px; line-height: 1.4; max-width: 700px; }
                .ph-header-subtitle { color: var(--accent-blue); font-weight: 600; margin-left: 10px; font-size: 0.6em; }
                
                .ph-header-actions {
                    display: flex;
                    gap: 10px;
                    align-items: center;
                    flex-shrink: 0; /* Acciones no se aplastan */
                    margin-top: 5px; /* Alineación visual con h1 */
                }

                .ph-tabs-container { display: flex; background: rgba(0,0,0,0.3); padding: 5px; border-radius: 12px; border: 1px solid var(--glass-border); gap: 5px; margin-bottom: 2rem; overflow-x: auto; scrollbar-width: none; -ms-overflow-style: none; flex-shrink: 0;}
                ...
                @media (max-width: 768px) {
                    ...
                    /* FIX: Empujar contenido abajo para que no lo tape el fixed header */
                    .ph-view-header { 
                    ...
                    /* FIX: Ajuste tamaño fuente título móvil para que quepa "TeamTowers Network" */
                    .ph-view-header h1 { 
                        font-size: 1.6rem; /* Reducción táctica */
                        letter-spacing: -1px;
                    }
                    .ph-header-subtitle {
                        font-size: 0.55em;
            ...
            <div class="ph-view-header">
                <div class="ph-view-header-title-group">
                    <h1>${config.title}${archetypeBadgeHtml}</h1>
