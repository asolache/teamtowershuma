...
export class KanbanRenderer {
    ...
    constructor(containerEl, options = {}) {
        ...
        this.options = Object.assign({
            ...
            isMacroMode: false // true si se inyecta dentro del Paper, false para la vista completa
        }, options);
    ...
    static getStyles() {
        return `
            /* ESTILOS PARA EL MODO TABLERO COMPLETO (3 COLUMNAS) */
            ...
            /* ESTILOS PARA MODO MACRO (INYECCIÓN EN PAPER) */
            ...
            .btn-focus { flex: 1; background: linear-gradient(135deg, rgba(0,176,255,0.1), rgba(0,176,255,0.2)); border: 1px solid var(--accent-blue); color: var(--accent-blue); text-align: center; text-decoration: none; padding: 10px; border-radius: 8px; font-weight: 900; transition: 0.3s; font-size: 0.85rem;}
    ...
    render() {
        ...
        allTasks = allTasks.filter(tx => {
            if (!tx.isWorkOrder) return true; // Legacy txs no tenían sprint
        ...
        allTasks.forEach(tx => {
            ...
            // Filtros visuales (Tangible/Intangible/Mis Tareas)
        ...
        // 1. RENDERIZADO MODO MACRO (PAPER)
        if (isMacroMode) {
            // En modo Macro, solemos querer renderizar solo las oportunidades (o todo junto sin columnas)
            const allCards = [...cols['oportunidades'], ...cols['en-curso'], ...cols['contabilizado']];
            if (allCards.length > 0) {
                this.container.innerHTML = `<div class="task-grid">${allCards.join('')}</div>`;
            } else {
                this.container.innerHTML = `<div class="empty-state">No hay tareas que mostrar para este contexto.</div>`;
            }
        } 
        // 2. RENDERIZADO MODO TABLERO (VISTA PROJECT)
        else {
                ...
                <div class="kanban-board-layout">
                    <div class="kanban-column" style="border-top: 3px solid #888;">
                        <div class="kanban-col-header" style="color: #ccc;">
                            ...
                            <span class="kanban-col-count">${cols['oportunidades'].length}</span>
                        </div>
                        <div class="kanban-col-body">${colOportunidades}</div>
                    </div>

                    <div class="kanban-column" style="border-top: 3px solid var(--accent-orange);">
                        <div class="kanban-col-header" style="color: var(--accent-orange);">
                            <span>⏳ En Curso / Auditoría</span>
                            <span class="kanban-col-count" style="color: white;">${cols['en-curso'].length}</span>
                        </div>
                        <div class="kanban-col-body">${colEnCurso}</div>
                    </div>

                    <div class="kanban-column" style="border-top: 3px solid var(--accent-green);">
                        <div class="kanban-col-header" style="color: var(--accent-green);">
                            <span>💎 Selladas (Ledger)</span>
                            <span class="kanban-col-count" style="color: white;">${cols['contabilizado'].length}</span>
                        </div>
    ...
    buildCardHTML(tx, flowData, project) {
        ...
        // ETIQUETAS Y BOTONES DEPENDIENDO DEL ESTADO
        if (tx.status === 'theoretical') {
            ...
            if (isPO) {
                actionHtml = `
                    ...
                    <button class="btn-push kb-action" data-action="push" ${hashAttr} title="Asignar a un miembro">👤 PUSH</button>
                `;
            } else {
            ...
            if (isMine) actionHtml = `<a href="/v8/paper?hash=${tx.hash}&legacy=${isLegacy}" class="btn-focus" data-link>▶ OMNI-PAPER</a>`;
            // 🔥 BOTÓN MÁGICO A2A: Si la tarea es de una IA y el usuario es el PO
