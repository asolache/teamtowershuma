...
// Componente para generar formularios dinámicos a partir de una lista de requisitos
        ...
        try {
            ...
            let html = `
                <style>
                    ...
                    label { display: block; margin-bottom: 0.25rem; font-weight: 600; color: var(--tt-secondary); }
                    input, textarea, select { width: 100%; padding: 0.5rem; border: 1px solid var(--border-light); border-radius: 4px; font-family: inherit; box-sizing: border-box; }
                    .requerido::after { content: " *"; color: var(--status-forca); }
                    .help-text { font-size: 0.8rem; color: var(--color-documentacion); margin-top: 0.25rem; }
                    button { background: var(--tt-accent); color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.2s; }
                    button:hover { transform: translateY(-2px); box-shadow: var(--shadow-tronc); }
                </style>
            ...
            html += `<button type="submit">${submitLabel}</button></form>`;
...
console.log('✅ tt-dynamic-form component loaded');
