// project.js - Vista detallada de proyecto con pestañas y selector de roles
...
function showRoleSelector(projectId) {
    console.log('👥 Abriendo selector de roles para:', projectId);
...
function saveProjectRoles(projectId, roles) {
    console.log('💾 Guardando roles para:', projectId, roles.length);
...
// ============================================
// FUNCIONES PARA RENDERIZAR PESTAÑAS
// ============================================
function renderTabs(currentTab, projectId) {
    return `
        <div style="display: flex; gap: 10px; border-bottom: 2px solid #e2e8f0; margin-bottom: 20px; flex-wrap: wrap;">
            <a href="#mapa" class="tab-link ${currentTab === 'mapa' ? 'active' : ''}" data-tab="mapa" data-project="${projectId}" style="padding: 10px 20px; text-decoration: none; color: ${currentTab === 'mapa' ? '#2563eb' : '#64748b'}; border-bottom: 2px solid ${currentTab === 'mapa' ? '#2563eb' : 'transparent'}; margin-bottom: -2px; cursor: pointer; font-weight: ${currentTab === 'mapa' ? 'bold' : 'normal'};">🗺️ Mapa de Valor</a>
            <a href="#roles" class="tab-link ${currentTab === 'roles' ? 'active' : ''}" data-tab="roles" data-project="${projectId}" style="padding: 10px 20px; text-decoration: none; color: ${currentTab === 'roles' ? '#2563eb' : '#64748b'}; border-bottom: 2px solid ${currentTab === 'roles' ? '#2563eb' : 'transparent'}; margin-bottom: -2px; cursor: pointer; font-weight: ${currentTab === 'roles' ? 'bold' : 'normal'};">👥 Roles</a>
            <a href="#entregables" class="tab-link ${currentTab === 'entregables' ? 'active' : ''}" data-tab="entregables" data-project="${projectId}" style="padding: 10px 20px; text-decoration: none; color: ${currentTab === 'entregables' ? '#2563eb' : '#64748b'}; border-bottom: 2px solid ${currentTab === 'entregables' ? '#2563eb' : 'transparent'}; margin-bottom: -2px; cursor: pointer; font-weight: ${currentTab === 'entregables' ? 'bold' : 'normal'};">📦 Entregables</a>
            <a href="#flujos" class="tab-link ${currentTab === 'flujos' ? 'active' : ''}" data-tab="flujos" data-project="${projectId}" style="padding: 10px 20px; text-decoration: none; color: ${currentTab === 'flujos' ? '#2563eb' : '#64748b'}; border-bottom: 2px solid ${currentTab === 'flujos' ? '#2563eb' : 'transparent'}; margin-bottom: -2px; cursor: pointer; font-weight: ${currentTab === 'flujos' ? 'bold' : 'normal'};">🔀 Flujos</a>
        </div>
    `;
}
                    ...
                    <tbody>
                        ${roles.map(rol => `
                            <tr>
                                ...
                                <td>
                                    <button onclick="removeRole('${project.id}', '${rol.id}')" style="background: none; border: none; cursor: pointer; color: #ef4444;">🗑️</button>
                                </td>
                            </tr>
                        `).join('')}
                    </tbody>
    ...
    return `
        <div>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                ...
                <button onclick="alert('Funcionalidad de añadir entregables próximamente (P-023)')" class="btn-primary" style="padding: 8px 16px; background: #10b981; color: white; border: none; border-radius: 30px; cursor: pointer;">
                    + Añadir Entregable
                </button>
            </div>
    ...
    return `
        <div>
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                ...
                <button onclick="alert('Funcionalidad de añadir flujos próximamente (P-024)')" class="btn-primary" style="padding: 8px 16px; background: #10b981; color: white; border: none; border-radius: 30px; cursor: pointer;">
                    + Añadir Flujo
                </button>
            </div>
            ...
            ` : `
                <div style="display: grid; gap: 15px;">
                    ${flujos.map(f => `
                        <div style="background: #f8fafc; padding: 15px; border-radius: 12px; display: flex; align-items: center; gap: 20px; flex-wrap: wrap;">
...
function removeRole(projectId, roleId) {
    if (confirm(`¿Eliminar el rol ${roleId} del proyecto?`)) {
        const state = window.store.getState();
        const projectIndex = state.projects.findIndex(p => p.id === projectId);
        
        if (projectIndex !== -1) {
            state.projects[projectIndex].roles = (state.projects[projectIndex].roles || []).filter(r => r.id !== roleId);
            window.store.saveState();
            location.reload();
        }
    }
    ...
    return `
        <div class="card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap;">
                ...
                <div>
                    <button onclick="window.router?.navigate('projects')" class="btn-back" style="padding: 8px 16px; background: #f1f5f9; border: none; border-radius: 30px; cursor: pointer;">← Volver a proyectos</button>
                </div>
            </div>
            ...
            <!-- Pestañas -->
            ${renderTabs(currentTab, project.id)}
            
            <!-- Contenido de las pestañas -->
            <div id="tab-content">
                ${currentTab === 'mapa' ? renderMapaTab(project, transactions) : ''}
                ${currentTab === 'roles' ? renderRolesTab(project) : ''}
                ${currentTab === 'entregables' ? renderEntregablesTab(project) : ''}
                ${currentTab === 'flujos' ? renderFlujosTab(project) : ''}
            </div>
        </div>
    `;
};
...
console.log('✅ Project page cargada con todas las funcionalidades');
