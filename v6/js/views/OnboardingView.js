...
export default class OnboardingView {
    ...
    async getHtml() {
        ...
        let suggestedHandle = '@nodo_nuevo';
        if (tempName) {
            const cleanName = tempName.normalize("NFD").replace(/[\u0300-\u036f]/g, "").split(' ')[0].toLowerCase();
            suggestedHandle = `@${cleanName}_${Math.floor(Math.random() * 999)}`;
        }
        ...
        return `
            <style>
                ...
                .form-row { display: flex; gap: 15px; margin-bottom: 1.5rem; flex-wrap: wrap;}
                ...
                .btn-primary { width: 100%; background: linear-gradient(45deg, var(--accent-blue), var(--accent-purple)); color: white; border: none; padding: 15px; border-radius: 8px; font-weight: bold; font-size: 1.1rem; cursor: pointer; transition: 0.2s; display: flex; justify-content: center; align-items: center; gap: 10px; margin-top: 1rem;}
                .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(179, 136, 255, 0.4); }
                .btn-primary:disabled { opacity: 0.5; cursor: not-allowed; transform: none; box-shadow: none; }
                
                .ai-status { display: none; text-align: center; margin-top: 1.5rem; color: var(--accent-orange); font-family: var(--font-mono); font-size: 0.85rem; font-weight: bold;}
                .error-msg { display: none; background: rgba(255, 82, 82, 0.1); border: 1px solid var(--accent-red); color: var(--accent-red); padding: 10px; border-radius: 8px; margin-top: 1rem; font-size: 0.85rem; text-align: center;}
                ...
                <div class="onb-card">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="email" id="inpEmail" class="form-control" value="${tempEmail}" ${tempEmail ? 'disabled' : ''} placeholder="correo@dominio.com">
                        </div>
                        <div class="form-group">
                    ...
                    <div id="onbErrorBox" class="error-msg"></div>

                    <button class="btn-primary" id="btnRegister">Acuñar Identidad Fractal ⚡</button>
                    <div class="ai-status" id="aiStatusMsg" style="animation: pulse 1.5s infinite;">Iniciando conexión con el Kernel...</div>
                </div>
            </div>
        `;
    }
    ...
    executeViewScript() {
        ...
        const errorBox = document.getElementById('onbErrorBox');
        ...
        btnRegister.addEventListener('click', async () => {
            ...
            if (!handle || !realName) {
                errorBox.innerText = "⚠️ El Alias y el Nombre son obligatorios.";
            ...
            // 1. SIMULACIÓN EDGE BACKEND (Anti-Colisión)
            statusMsg.innerText = "🌐 Validando disponibilidad del @alias en la Permaweb...";
            
            const edgeBackendSimulation = new Promise((resolve, reject) => {
                setTimeout(() => {
                    const state = store.getState();
                    // Comprobamos si el alias ya está cogido en nuestra "BD Global"
                    const aliasTakenLocal = state.globalUsers.find(u => u.id === handle.toLowerCase());
                    if (aliasTakenLocal) {
                        reject(new Error(`El alias ${handle} ya pertenece a otro usuario. Por favor, escoge otro.`));
                    } else {
                        resolve({ status: "OK" });
                    }
                }, 1200); 
            ...
            // 2. INVOCACIÓN IA (Si hay API Key y texto)
            ...
            if (apiKey && cvText.length > 20) {
                try {
                    ...
                    console.warn("Fallo IA en onboarding", e);
                }
            }

            // 3. REGISTRO LOCAL Y LOGIN
            statusMsg.style.animation = 'none';
            statusMsg.style.color = 'var(--accent-green)';
            statusMsg.innerText = "✅ Inyectando Nodo en el Kernel... Accediendo.";
            ...
            const uIdx = currentState.globalUsers.findIndex(u => u.id === handle.toLowerCase());
            if (uIdx > -1) {
            ...
            await store.dispatch({ type: 'LOGIN_USER', payload: { userId: handle.toLowerCase() } });
