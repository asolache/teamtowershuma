...
export default class TestsView {
                ...
                <main class="workspace">
                    ...
                    <div class="test-container">
                        <div class="test-header">
                            <h1>KERNEL v7.2 VALIDATION</h1>
                            <p style="color: var(--text-muted);">Ejecutando 62 validaciones: SHA-256, Privacidad, Triple Entrada, RBAC, Slicing Pie, Identidad Fractal</p>
                        </div>
    ...
    executeViewScript() {
        ...
        btn.addEventListener('click', async () => {
            btn.disabled = true;
            terminal.innerHTML = '<div style="color: var(--accent-blue); margin-bottom: 15px; font-weight: bold;">> Iniciando motor de aserciones V7.2...</div>';
            ...
            try {
                ...
                // -------------------------------------------------------------
                // NUEVOS TESTS V7.2: PRIVACIDAD Y GOBERNANZA (FASE 1)
                // -------------------------------------------------------------
                ...
                assert(pPriv.invitations !== undefined, "Registro de Invitaciones (Mail) inicializado", "DATA");
                ...
                assert(store.canUserViewProject(PID_PRIV, 'usr_alvaro_001', 'ecosystem-owner') === true, "El Master Architect puede ver redes privadas (Bypass root)", "RBAC-V7.2");
                assert(store.canUserViewProject(PID_PRIV, DYN_HACKER_ID, 'network-user') === false, "Firewall: Usuario no invitado bloqueado del mapa privado", "RBAC-V7.2");

                await store.dispatch({ type: 'ADD_USER', payload: { projectId: PID_PRIV, userId: DYN_HACKER_ID, name: 'Hacker' } });
                assert(store.canUserViewProject(PID_PRIV, DYN_HACKER_ID, 'network-user') === true, "Firewall: Usuario invitado (miembro) obtiene acceso al mapa", "RBAC-V7.2");

                await store.dispatch({ type: 'LOG_INVITATION', payload: { projectId: PID_PRIV, email: 'test@daohack.com' } });
                assert(store.getState().projects.find(x => x.id === PID_PRIV).invitations.length === 1, "Sistema audita y registra envíos de invitaciones por email", "COMMS");
                ...
                if(passed === total) {
                    ...
                    terminal.innerHTML += `
                        <div style="margin-top: 25px; padding: 20px; border: 1px solid ${finalColor}; background: rgba(0, 230, 118, 0.1); border-radius: var(--border-radius-md); text-align: center; animation: fadeIn 0.5s ease-in;">
                            <h2 style="color: ${finalColor}; margin: 0; font-size: 2rem;">🚀 KERNEL v7.2 VALIDADO AL 100%</h2>
                            <p style="color: white; margin-top: 10px; font-size: 1.1rem;">Los ${total} vectores de criptografía asíncrona, RBAC, Privacidad y Slicing Pie han sido superados.</p>
                        </div>
                    `;
