...
export default class SettingsView {
    ...
    async getHtml() {
        ...
        return `
            <style>
                ...
                .modal { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 1000; display: none; align-items: flex-start; justify-content: center; backdrop-filter: blur(10px); overflow-y: auto; padding: 2rem 0;}
                        ...
                        ${Object.entries(customSectores).map(([key, roles]) => `
                            <div class="sector-card">
                                ...
                                <div style="display:flex; gap:10px; margin-top:1.5rem;">
                                    <button class="btn-save btn-use-template" data-sector="${key}" style="flex:2; background:var(--accent-green); color:black; font-size:0.8rem;">🚀 Usar Plantilla</button>
                    ...
                    <div class="ontology-grid">
                        ${Object.entries(nativeSectores).map(([key, data]) => {
                            ...
                            <div class="sector-card native">
                                ...
                                <button class="btn-save btn-use-template" data-sector="${key}" style="width:100%; margin-top:1.5rem; background:rgba(0, 230, 118, 0.2); color:var(--accent-green); font-size:0.8rem;">🚀 Usar Plantilla</button>
                            </div>
                        `}).join('')}
    ...
    bindContentEvents() {
        ...
        if (this.tab === 'ontology') {
            // Usar Plantilla (Navega a CreateView)
    ...
    openOntologyModal(sectorKey, state) {
        ...
        // Construir array inicial de roles dinámicos
        let modalRoles = [];
        if (sectorData) {
            ...
            // Si es nuevo, cargar los 5 niveles por defecto para que sea más fácil
            ...
            return modalRoles.map((r, index) => {
                ...
                return `
                    <div class="role-block" data-uid="${r.uid}" style="background:rgba(255,255,255,0.02); padding:1.5rem; border-radius:8px; border-left:4px solid var(--accent-blue); position:relative;">
                        <button type="button" class="btn-remove-role" data-uid="${r.uid}" style="position:absolute; top:10px; right:10px; background:var(--accent-red); color:white; border:none; border-radius:4px; padding:2px 8px; cursor:pointer; font-size:0.7rem;">🗑️ Eliminar Rol</button>
                        
                        <div style="display:grid; grid-template-columns: 1fr 2fr 1fr 1fr; gap:15px; align-items:end; margin-bottom:15px;">
                            <div>
                        ...
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                            <div>
                                ...
                                <textarea class="form-control inp-prompt" placeholder="Contexto: Eres el encargado de supervisar..." style="height:80px; font-size:0.8rem;">${r.ai_prompt}</textarea>
                            </div>
                            <div>
                                ...
                                <textarea class="form-control inp-deliv" placeholder="@dosos | 10 | tangible | Definir Arquitectura" style="height:60px; font-size:0.8rem; color:var(--accent-green);">${delivs}</textarea>
                            </div>
                        </div>
                    </div>
                `;
        ...
        content.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
                <div>
                    ...
                    <p style="color:var(--text-muted); font-size:0.85rem; margin-top:5px;">Configura los Roles, Arquetipos y Flujos predefinidos tal y como harías al crear una red viva.</p>
                </div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:2rem; background:rgba(0,0,0,0.5); padding:1rem; border-radius:8px;">
                <span style="font-size:0.8rem; color:var(--text-muted);">Puedes reusar esta plantilla para crear infinitos proyectos idénticos.</span>
        ...
        // Lógica de "Eliminar Rol" y "Añadir Rol" (Delegación de Eventos)
        ...
        rolesContainer.addEventListener('click', (e) => {
            if (e.target.classList.contains('btn-remove-role')) {
                ...
                rolesContainer.innerHTML = renderDynamicRoles(); // Re-render rápido
            }
        });
        ...
        document.getElementById('btn-save-sector-action').addEventListener('click', async () => {
            ...
            // Leer el DOM directamente asegura que cogemos los últimos valores tecleados
            document.querySelectorAll('.role-block').forEach((block, idx) => {
                ...
                const uniqueKey = `role_${Date.now()}_${idx}`; // Clave única para soportar múltiples roles del mismo nivel
                rolesData[uniqueKey] = {
                    levelId: levelId,
