...
export default class DashboardView {
    ...
    async getHtml() {
                ...
                <div class="app-layout">
                    ...
                    <main class="workspace" style="justify-content:center; align-items:center; display:flex; height:100vh; background: var(--bg-dark);">
                        <div style="text-align:center; padding: 4rem; border: 1px dashed #333; border-radius: 16px; background: rgba(255,255,255,0.02); backdrop-filter: blur(10px);">
                             ...
                             <a href="/v5/create" data-link class="btn-lux-primary" style="text-decoration:none;">➕ Instanciar Proyecto</a>
                        </div>
                    </main>
                </div>
            `;
            ...
            : sillasVacias.map(r => `
                <div class="vacante-card">
                    <div style="display:flex; align-items:center; gap:12px;">
                        <div style="font-size:1.5rem; filter:drop-shadow(0 0 4px rgba(255,255,255,0.2));">${this.getIcon(r.levelId)}</div>
                        <div>
                            <div class="vacante-name">${r.name}</div>
                            <div class="vacante-meta">${r.levelId} | FMV: <span style="color:var(--accent-green);">${r.fmv}€/h</span></div>
                        </div>
                    </div>
        ...
        const headerConfig = {
            ...
            tagline: "Resumen Operativo y Auditoría de Red"
        };
        ...
        return `
            <style>
                ...
                /* =========================================================
                   ...
                   ========================================================= */
                .workflow-schema { display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.4); border: 1px solid #222; border-radius: 16px; padding: 1.5rem 2rem; margin-bottom: 2rem; overflow-x: auto; scrollbar-width: none;}
                .schema-step { display: flex; flex-direction: column; align-items: center; text-decoration: none; text-align: center; gap: 8px; transition: transform 0.2s, filter 0.2s; filter: grayscale(50%) opacity(0.8);}
                .schema-step:hover { transform: translateY(-3px); filter: grayscale(0%) opacity(1);}
                .s-icon { font-size: 2rem; background: rgba(255,255,255,0.05); width: 60px; height: 60px; display: flex; justify-content: center; align-items: center; border-radius: 50%; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 4px 15px rgba(0,0,0,0.5);}
                .s-text { color: white; font-weight: 900; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1px; line-height: 1.3;}
                .s-text span { color: var(--accent-blue); font-size: 0.7rem; font-family: var(--font-mono); }
                .schema-arrow { color: #444; font-size: 1.5rem; font-weight: bold; }

                /* =========================================================
                   PRESENTACIÓN COMPACTA
                   ========================================================= */
                .presentation-box { margin-bottom: 3rem; background: rgba(255,255,255,0.015); padding: 1.5rem 2rem; border-radius: 16px; border: 1px solid var(--glass-border); position: relative; border-left: 4px solid var(--accent-purple); backdrop-filter: blur(10px);}
                .presentation-text { color: #ccc; font-size: 0.95rem; line-height: 1.6; overflow: hidden; transition: max-height 0.4s ease; font-family: var(--font-main);}
                .presentation-text.collapsed { max-height: 70px; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; }
                .btn-read-more { background: none; border: none; color: var(--accent-purple); font-weight: 900; cursor: pointer; padding: 10px 0 0 0; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px;}

                /* =========================================================
                   KPIS COMPACTAS
                   ========================================================= */
                .kpi-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 3rem; }
                .kpi-card { background: linear-gradient(145deg, rgba(0,0,0,0.6), rgba(15,15,20,0.8)); border: 1px solid var(--glass-border); padding: 1.5rem; border-radius: 16px; text-align: center; transition: 0.3s; box-shadow: inset 0 2px 10px rgba(255,255,255,0.01);}
                .kpi-card:hover { transform: translateY(-3px); box-shadow: 0 10px 25px rgba(0,0,0,0.4); border-color: rgba(255,255,255,0.1);}
                .kpi-val { font-size: 2rem; font-weight: 900; display: block; margin-bottom: 5px; font-family: var(--font-mono); line-height: 1;}
                .kpi-lbl { font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 1px; font-weight: bold; }
                ...
                .panel-title { color: white; margin-top: 0; font-size: 1.2rem; margin-bottom: 1rem; display: flex; align-items: center; gap: 10px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.5px;}
                
                /* VACANTES */
                .vacantes-list { max-height: 300px; overflow-y: auto; padding-right: 10px;}
                .vacante-card { display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.4); border: 1px solid #333; padding: 1rem; border-radius: 12px; margin-bottom: 10px; transition: transform 0.2s;}
                .vacante-card:hover { transform: translateX(5px); border-color: #555; background: rgba(255,255,255,0.02);}
                .vacante-name { color: white; font-weight: bold; font-size: 0.95rem;}
                .vacante-meta { font-size: 0.75rem; color: #888; font-family: var(--font-mono); margin-top: 4px; }
                .btn-invite { background: transparent; border: 1px solid var(--accent-blue); color: var(--accent-blue); padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 0.75rem; font-weight: bold; transition: all 0.2s; text-transform: uppercase;}
                .btn-invite:hover { background: var(--accent-blue); color: black; box-shadow: 0 0 10px rgba(0,176,255,0.3);}

                /* BOTONES IA PREMIUM */
                .btn-ai-action { width: 100%; padding: 1.2rem; border-radius: 12px; border: 1px solid rgba(224, 64, 251, 0.3); background: linear-gradient(145deg, rgba(0,0,0,0.6), rgba(224, 64, 251, 0.05)); color: white; text-align: left; cursor: pointer; transition: all 0.3s; margin-bottom: 12px; position: relative; overflow: hidden;}
                ...
                .btn-ai-action:hover { border-color: var(--accent-purple); transform: translateY(-3px); box-shadow: 0 10px 20px rgba(224,64,251,0.15);}
                ...
                .btn-ai-action span { color: #aaa; font-size: 0.8rem; line-height: 1.4; display: block;}

                .status-ok { padding: 1.5rem; background: rgba(0, 230, 118, 0.05); border: 1px solid rgba(0, 230, 118, 0.2); border-radius: 12px; color: var(--accent-green); text-align: center; font-weight: bold; font-size: 0.9rem;}
                ...
                .btn-lux-primary { background: linear-gradient(135deg, var(--accent-blue), var(--accent-purple)); color: white; padding: 10px 20px; font-weight: 800; border-radius: 8px; border:none; cursor: pointer; transition: 0.3s;}
                .btn-lux-primary:hover { filter: brightness(1.2); box-shadow: 0 5px 15px rgba(224, 64, 251, 0.3);}
                ...
                @media (max-width: 768px) {
                    .workspace { padding: 90px 1.5rem 3rem 1.5rem; }
                    .workflow-schema { flex-direction: column; gap: 15px; padding: 2rem 1rem; }
                ...
                <main class="workspace">
                    ...
                    <div class="workflow-schema">
                        ...
                        <a href="/v5/focus" data-link class="schema-step" title="Ejecuta con Proof of Work">
                            <div class="s-icon">🍅</div>
                            <div class="s-text">3. Ejecutar<br><span>(Focus)</span></div>
                        </a>
                        <div class="schema-arrow">→</div>
                        <a href="/v5/ledger" data-link class="schema-step" title="Acumula Slices inmutables">
                            <div class="s-icon">⚖️</div>
                            <div class="s-text">4. Cobrar<br><span>(Ledger)</span></div>
                        </a>
                    </div>
                    ...
                    <div class="content-row">
                        ...
                        <div class="panel-box" style="border-color: rgba(224, 64, 251, 0.2); background: rgba(20, 10, 25, 0.6);">
                            <h2 class="panel-title" style="color:var(--accent-purple);">🔮 Orquestador Legal IA</h2>
                            <p style="color:#888; font-size:0.85rem; margin-bottom:1.5rem; line-height:1.4;">Agentes cognitivos para auditar el estado del proyecto y emitir pactos de socios en tiempo real.</p>
            ...
            <div id="aiModal" class="modal-ia">
                <div class="modal-ia-content">
                    <div style="padding:20px 30px; border-bottom:1px solid #333; display:flex; justify-content:space-between; align-items:center;">
                        <h2 id="aiModalTitle" style="margin:0; font-size:1.2rem; color:var(--accent-purple); font-weight:900;">Procesando...</h2>
