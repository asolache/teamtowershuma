...
export default class FocusView {
    ...
    executeViewScript() {
        ...
        if (allMyTasks.length === 0) {
            ...
            // Llenar Omni-Selector (AQUÍ ESTABA EL ERROR SINTÁCTICO REPARADO)
            this.dom.omniContainer.style.display = 'flex';
            this.dom.omniSelector.innerHTML = allMyTasks.map((t, idx) => 
                `<option value="${t.hash}" data-pid="${t.projectId}" ${idx===0?'selected':''}>${t.entregable} (${t.projectName})</option>`
            ).join('');
