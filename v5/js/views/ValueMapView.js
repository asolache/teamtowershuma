...
export default class ValueMapView {
    ...
    async getHtml() {
        ...
        return `
            <style>
                /* CSS ESPECÍFICO DEL MOTOR DE DIBUJO (NO ELIMINAR) */
                .vna-controls-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; gap: 15px; flex-wrap: wrap; background: rgba(255,255,255,0.02); padding: 15px; border-radius: 16px; border: 1px solid var(--glass-border); backdrop-filter: blur(5px); }
                
                .map-container { flex: 1; position: relative; overflow: hidden; border: 1px solid rgba(255,255,255,0.05); border-radius: 20px; background: linear-gradient(180deg, rgba(15,15,20,0.9) 0%, rgba(5,5,8,1) 100%); box-shadow: inset 0 0 100px rgba(0,0,0,0.8); min-height: 500px; width: 100%;}
                ...
                .edge-sick { stroke: var(--accent-red) !important; filter: drop-shadow(0 0 8px var(--accent-red)); }
                .edge-temp { stroke: var(--accent-orange); stroke-width: 4; stroke-dasharray: 6, 6; animation: dashAnim 5s linear infinite; }
                ...
                .node-circle { position: relative; border-radius: 50%; display: flex; flex-direction: column; justify-content: center; align-items: center; transition: all 0.3s; background: rgba(20, 20, 25, 0.8); backdrop-filter: blur(15px); border: 3px solid rgba(255,255,255,0.1); color: white; width: 85px; height: 85px; box-sizing: border-box;}
                ...
                .node-fmv { font-size: 1.1rem; font-weight: 900; font-family: var(--font-mono); }
                ...
                .btn-zoom { background: rgba(0,0,0,0.6); border: 1px solid #444; color: white; border-radius: 10px; width: 45px; height: 45px; font-size: 1.4rem; cursor: pointer; display: flex; justify-content: center; align-items: center; transition: 0.2s;}
                .btn-zoom:hover { background: rgba(255,255,255,0.1); border-color: white;}
            ...
            <div class="app-layout">
                ...
                <main class="workspace">
                    ...
                    <div id="view-edit" class="tab-content">
                        ${isPO ? `
                            <div class="vna-tip">
                                <div id="editTipText">💡 <b>Modo Arquitecto:</b> Haz click en un nodo y luego en otro para trazar una tubería permanente.</div>
                        ...
                        ` : '<div class="panel">🔒 Solo el Project Owner puede editar la topología.</div>'}
                    </div>
                ...
                <div class="modal-overlay" id="txBuilderModal">
                    <div class="modal-content">
                        ...
                        <div class="form-group"><label>Catálogo V10</label><select id="selTemplate" class="form-control"></select></div>
                        <div class="form-group"><label>Horas Est.</label><input type="number" id="inpHoras" class="form-control"></div>
                        <div class="form-group"><label>Tipo</label><select id="selType" class="form-control"><option value="tangible">Tangible</option><option value="intangible">Intangible</option></select></div>
                        <div class="form-group"><label>Concepto</label><input type="text" id="inpDesc" class="form-control"></div>
                        <button class="btn-primary" style="width:100%; margin-top:2rem;" id="btnAddFlow">Guardar Tubería</button>
                        <button class="btn-outline" style="width:100%; margin-top:1rem;" id="btnCloseTxBuilder">Cancelar</button>
                    </div>
                </div>
                ...
                <div class="modal-overlay" id="addNodeModal">
                    <div class="modal-content">
                        ...
                        <div class="form-group"><label>Nivel</label><select id="selNewNodeLevel" class="form-control">
                        ...
                        <button class="btn-primary" style="width:100%; margin-top:2rem;" id="btnConfirmNode">Añadir</button>
                        <button class="btn-outline" style="width:100%; margin-top:1rem;" id="btnCancelNode">Cerrar</button>
                    </div>
                </div>
                ...
                <div class="modal-overlay" id="inspectorModal">
                    <div class="modal-content">
                        ...
                        <button class="btn-primary" style="width:100%; margin-top:1rem;" id="btnSaveRole">Actualizar</button>
                        <button class="btn-outline" style="width:100%; margin-top:1rem; color:var(--accent-red);" id="btnDeleteRole">Archivar</button>
    ...
    executeViewScript() {
        ...
        // Lógica de Tabs DRY (V12)
        window.addEventListener('ph-tab-changed', (e) => {
            document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'));
            document.getElementById(`view-${e.detail.tabId}`)?.classList.add('active');
        ...
        // Tooltip y Drag logic original de Alvaro (Mantenida 100%)
        ...
        document.getElementById('btnCloseTxBuilder')?.addEventListener('click', () => this.dom.txBuilderModal.style.display = 'none');
        document.getElementById('btnCancelNode')?.addEventListener('click', () => this.dom.addNodeModal.style.display = 'none');
        document.getElementById('btnCloseInspector')?.addEventListener('click', () => this.dom.inspectorModal.style.display = 'none');
    ...
    renderSequence(highlight = -1) { 
        const p = store.getState().projects.find(x => x.id === this.activeProjectId);
        const flows = p.vna_flows?.length ? p.vna_flows : p.transactions || [];
        document.getElementById('sequenceList').innerHTML = flows.map((f, i) => `
    ...
    handleNodeClick(id) {
        ...
        else if (this.flowFromId !== id) { this.flowFromId = null; }
    }
