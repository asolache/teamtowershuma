...
export const HomeView = {
    render: () => {
        ...
        // --- CÁLCULOS DE ECOSISTEMA ---
        const totalLiquidez = projects.reduce((sum, p) => 
            sum + p.transactions.reduce((s, t) => s + t.liquidación, 0), 0);
        
        const proyectosEnRiesgo = projects.filter(p => store.calculateResilience(p.id) < 30).length;
        
        const totalHoras = projects.reduce((sum, p) => 
            sum + p.transactions.reduce((s, t) => s + t.horas, 0), 0);

        return `
            <div style="max-width: 1000px; margin: 0 auto; padding: 40px 20px;">
                <header style="margin-bottom: 40px;">
                    <h1 style="color: #f0f6fc; font-size: 2rem; margin-bottom: 10px;">SOS Dashboard <span style="color: #30363d; font-weight: normal;">v4.2</span></h1>
                ...
                <section style="background: #0d1117; border: 1px solid #30363d; padding: 25px; border-radius: 12px; margin-bottom: 40px;">
                    ...
                    <div style="display: grid; grid-template-columns: 2fr 1.5fr 1fr; gap: 15px;">
                        <input id="new-p-name" type="text" placeholder="Nombre del proyecto..." 
                               style="background: #161b22; border: 1px solid #30363d; color: white; padding: 12px; border-radius: 6px;">
                        ...
                        <button onclick="window.createProject()" 
                                style="background: #238636; color: white; border: none; font-weight: bold; border-radius: 6px; cursor: pointer;">
                            Inicializar
                        </button>
                    </div>
                </section>
                ...
                <div style="display: grid; grid-template-columns: 1fr; gap: 10px;">
                    ...
                    ${projects.map(p => {
                        ...
                        <div onclick="location.hash='#/project/${p.id}'" 
                             style="background: #161b22; border: 1px solid #30363d; padding: 15px 25px; border-radius: 8px; display: flex; align-items: center; justify-content: space-between; cursor: pointer; transition: transform 0.1s, border-color 0.2s;"
                ...
                <div style="margin-top: 50px; text-align: center;">
                    <button onclick="store.dispatch({type:'RESET_DATABASE'})" style="background: none; border: 1px solid #f85149; color: #f85149; padding: 5px 15px; border-radius: 4px; font-size: 0.7rem; cursor: pointer; opacity: 0.5;">
                        HARD RESET (Limpiar todo el ecosistema)
                    </button>
                </div>
            </div>
        `;
