...
document.addEventListener('click', (e) => {
    ...
    // Crear una nueva Identidad (Usuario)
    if (e.target.id === 'btn-create-user') {
        ...
        // Aseguramos que siempre lleve la @
        const userId = rawId.startsWith('@') ? rawId : `@${rawId}`;

        try {
            store.dispatch({
                type: 'ADD_USER',
                payload: { id: userId, projectId, name, walletOrSocial: wallet }
            });
            document.getElementById('app').innerHTML = ProjectAccountingView.render(projectId);
        } catch (err) {
            alert("❌ " + err.message);
        ...
        return `
            <div class="container fade-in">
                <div class="panel-surface" style="margin-bottom: 30px; border-left: 4px solid var(--accent-green);">
                    ...
                    ${totalSlices === 0 ? `
                        <div class="text-center text-muted" style="padding: 20px;">Aún no hay Slices congelados. El Ledger está limpio.</div>
                    ` : `
                ...
                <div class="grid-layout">
                    <main>
                        <h3 style="margin-bottom: 20px; color: var(--text-heading);">👥 Inyectar Nodos en la Red</h3>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                            <div class="panel" style="border-color: var(--accent-purple);">
                                ...
                                <p class="text-small text-muted" style="margin-bottom: 15px;">Registra a un humano en la base de datos global.</p>
                                ...
                                <button id="btn-create-user" data-pid="${projectId}" class="btn btn-primary btn-block" style="background: var(--accent-purple); margin-top: 15px;">Registrar Identidad</button>
                            </div>

                            <div class="panel" style="border-color: var(--accent-blue);">
                                <h4 style="color: var(--accent-blue); margin-top: 0;">2. Vincular a Ontología</h4>
                                <p class="text-small text-muted" style="margin-bottom: 15px;">Conecta la identidad (@id) con un rol del Mapa.</p>
                                
                                <label class="form-label">Selecciona Identidad (@id):</label>
                                ...
                                <button id="btn-assign-role" data-pid="${projectId}" class="btn btn-primary btn-block" style="background: var(--accent-blue); margin-top: 15px;">Vincular Nodo</button>
                            </div>
                        </div>

                        <h4 style="margin-top: 30px; margin-bottom: 15px;">Nodos Operativos Actuales</h4>
                        ${asignaciones.length === 0 ? '<p class="text-muted text-small">No hay nodos vinculados.</p>' : `
                            <div style="display: flex; flex-direction: column; gap: 10px;">
                                ${asignaciones.map(a => {
                                    ...
                                    if(!role) return '';
                                    return `
                                    <div class="panel-surface" style="display: flex; justify-content: space-between; align-items: center; padding: 12px; border-left: 2px solid var(--border-color);">
                                        <div>
                    ...
                    <aside>
                        ...
                        <p class="text-small text-muted" style="margin-bottom: 15px;">Registro inmutable. Cada entrada representa *Equity* sellado.</p>
                        
                        ${ledger.length === 0 ? `
                            <div class="panel text-center" style="padding: 20px; border-style: dashed;">
