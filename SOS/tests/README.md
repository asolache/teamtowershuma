...
| `test-joc.mjs` | vedes 110 i 111 | Que el joc ensenyi el SOS i no el decori: que l'única manera de perdre sigui la cremada, que formar algú compti el doble, que el coneixement només s'obri ensenyant, que McGragor **ofereixi** una cosa que surt a compte avui, i que Molekulàndia no s'aconsegueixi ocupant el mapa. I la capa de la plaça: que plantar amb el dit obri la dinàmica al node —no dues comptabilitats—, que els minijocs paguin en hores i gent i no en punts, que els vilans tinguin el poder del còmic, i que la capçalera no es talli ni es mengi la plaça a 320–430 px. I la introducció (veda 117): que el laboratori del tercer pas pinti **el que diuen `capacitatDe` i `carregaDe`** i no una còpia, que comenci en verd i es pugui arribar a cremar tocant botons, i que els vilans, els nivells i les accions surtin del joc i no d'una frase escrita a mà |
| `test-portada.mjs` | vedes 107 i 113 | Que la primera pantalla de `teamtowershuma.com` digui el negoci sense llegir: dos castells amb el mateix tronc i dues bases, el mateix clic als dos costats, el número i el dibuix dient el mateix, tot dins del plec a 1280, 1440 i 390 px, i cap sigla —«VNA»— a la cara de qui acaba d'arribar |
| `test-escola.mjs` | veda 114 | El programa d'escola: que un cromo acabat de fer no tingui cap estrella, que ningú se'n pugui posar una a si mateix, que la posi qui ha rebut l'ajuda i no el mestre, que els poders i les armes siguin del catàleg del SOS, i que **cap nom d'infant** pugui viatjar —comprovat sobre el que es construeix, no sobre el que es declara |
| `test-intro.mjs` | veda 115 | La intro: que les dues vistes siguin **un sol tall** —mateixos plans, mateixes durades, rètol a totes dues llengües—, que les dues pells s'animin amb la mateixa crida, que canviar de vista no reiniciï res, que el castell caigui amb quatre i aguanti amb catorze, que cada pla duri un nombre rodó de mitjos compassos del tema (80,2 ppm), que mani el rellotge de l'àudio i no el del navegador, i que **sense el fitxer de música** la peça funcioni sencera |
| `test-vna.mjs` | veda 116 | El mapa de valor amb la colla castellera: que hi hagi sempre les **dues menes** de lliurament, que qui només dona es **calculi** del graf i no s'assenyali a mà, que treure un rol tingui un cost comptat, que el text de cada rol sigui el de la portada paraula per paraula, i que el pas 8 canviï els noms sense tocar el graf |
